      ...
      <section class="grid" style="grid-template-columns:1.15fr .85fr; align-items:start;">
        <article class="card">
          ...
          <div class="grid grid-3" style="gap:14px;">
            ${filtered.map(p => `<article class="card" style="padding:14px; border-radius:16px;"><div style="display:flex; justify-content:space-between; gap:10px; align-items:start;"><div><div style="font-weight:800;">${p.nombre}</div><div class="muted" style="font-size:.88rem; margin-top:4px;">SKU: ${p.sku}</div><div class="muted" style="font-size:.88rem;">${p.categoria}</div></div><span class="status-tag ${p.stock <= 5 ? 'danger' : p.stock <= 10 ? 'warn' : 'soft-blue'}">Stock ${p.stock}</span></div><div class="kpi-value" style="font-size:1.4rem;">$${p.precio.toFixed(2)}</div><button class="btn btn-primary add-product-btn" data-id="${p.id}" style="width:100%;">Agregar</button></article>`).join('') || `<article class="card"><div class="muted">No se encontraron productos.</div></article>`}
          </div>
        </article>
        <article class="card">
          <h3>Carrito</h3>
          ...
          <div class="status-list" style="margin-top:16px;">
            ${state.cart.items.length ? state.cart.items.map(item => `<div class="status-item" style="align-items:flex-start;"><div style="min-width:0; flex:1;"><div style="font-weight:700;">${item.nombre}</div><div class="muted" style="font-size:.85rem;">$${item.precio.toFixed(2)} c/u · stock ${item.stock}</div></div><div style="display:flex; gap:8px; align-items:center;"><input class="qty-input" data-id="${item.id}" type="number" min="1" max="${item.stock}" value="${item.cantidad}" style="width:76px; padding:10px 12px;" /><button class="btn btn-secondary remove-item-btn" data-id="${item.id}">Quitar</button></div></div>`).join('') : `<div class="status-item"><span class="muted">Aún no hay productos en el carrito.</span></div>`}
          </div>
...
export function bindVentas(render) {
  document.getElementById('salesSearch')?.addEventListener('input', e => { state.salesQuery = e.target.value; render(); });
  document.getElementById('clearSearchBtn')?.addEventListener('click', () => { state.salesQuery = ''; render(); });
  document.getElementById('newSaleBtn')?.addEventListener('click', () => { state.cart = emptyCart(); render(); showToast('Venta nueva lista.'); });
  document.querySelectorAll('.add-product-btn').forEach(btn => btn.addEventListener('click', () => {
    const p = sampleProducts.find(x => x.id === btn.dataset.id);
    if (!p) return;
    const found = state.cart.items.find(i => i.id === p.id);
    if (found) {
      if (found.cantidad + 1 > p.stock) return showToast('No hay suficiente stock.');
      found.cantidad += 1;
    } else {
      if (p.stock <= 0) return showToast('Producto sin stock.');
      state.cart.items.push({ id: p.id, nombre: p.nombre, precio: p.precio, cantidad: 1, stock: p.stock, sku: p.sku, barcode: p.barcode });
    }
    render();
  }));
  document.querySelectorAll('.remove-item-btn').forEach(btn => btn.addEventListener('click', () => { state.cart.items = state.cart.items.filter(i => i.id !== btn.dataset.id); render(); }));
  document.querySelectorAll('.qty-input').forEach(input => input.addEventListener('change', () => {
    ...
    if (!item) return;
    item.cantidad = Math.max(1, Math.min(Number(input.value) || 1, item.stock));
    render();
  }));
